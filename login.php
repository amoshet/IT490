...
$email = $_POST["email"];
$type = "login";
$credential = array("email"=>$email,"password"=> $password, "type"=>$type);
...
if (isset($_POST["login"])) {
    ...
    if ($isValid) {
        $db = getDB();
        if (isset($db)) {
            $stmt = $db->prepare("SELECT id, email, username, password from Users WHERE email or Username = :email LIMIT 1");
...
SELECT Roles.name FROM Roles JOIN UserRoles on Roles.id = UserRoles.role_id where UserRoles.user_id = :user_id and Roles.is_active = 1 and UserRoles.is_active = 1");
                    ...
                    flash("Have you forgotten your password?<br>");
                }
            }
            else {
                flash("Invalid Email or Username<br>");
            }
        }
    }
    else {
